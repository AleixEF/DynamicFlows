...
def compute_validation_loss(nf, folder2load, n_val_batches):
    loss = 0
...
def validate(nf, sequences_batch):
    with torch.no_grad():
        loglike = nf.loglike_sequence(sequences_batch)
        loss = -torch.mean(loglike)
    return loss.item()
        

def train(nf, optimizer, sequences_batch):
    loglike = nf.loglike_sequence(sequences_batch)
    loss = -torch.mean(loglike)
...
models = [rnnflows.NormalizingFlow(frame_dim, encoding_dim, hidden_dim, 
...
for model_idx, nf in enumerate(models):
    ...
    for epoch in range(n_epochs):
        ...
        for batch_idx in range(n_train_batches):
            ...
            batch_loss = train(nf, optim, train_batch)
            train_loss += batch_loss
            
        train_loss /= n_train_batches
        val_loss = compute_validation_loss(nf, val_path, 
                                           n_val_batches)
...
plot_loss(train_loss_evol, val_loss_evol, "rnn_losses")
plt.show()
