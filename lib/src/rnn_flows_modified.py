...
class NormalizingFlow(nn.Module):
    def __init__(self, frame_dim, encoding_dim, hidden_layer_dim, b_mask=None,   
                 num_flow_layers=2, num_hidden_layers=1, toeplitz=True, model_type='rnn', device='cpu'):
        ...
        self.lambda_ = nn.Parameter(torch.Tensor([0.9])) #.requires_grad_(True)
        #self.lambda_ = nn.Parameter(torch.rand(1,1)) #.requires_grad_(True)
    ...
    def loglike_sequence(self, x_sequence, seq_lengths=None):
        ...
        self.slope_old_layers = [[None, None] for _ in range(self.num_flow_layers)]
        self.intercept_old_layers = [[None, None] for _ in range(self.num_flow_layers)]

        for frame_instant, x_frame in enumerate(x_sequence):
            
            loglike_frame, self.slope_old_layers, self.intercept_old_layers = self.loglike_frame(
                x_frame, h_encoding, self.slope_old_layers, self.intercept_old_layers, self.lambda_)
    ...
    def loglike_frame(self, x_frame, h_encoding, slope_old_layers=None, intercept_old_layers=None, lambda_=None):
        ...
        for i, flow_layer in enumerate(reversed(self.flow_layers)):

            # the first mask 1-b only modifies the first features of each frame
            z_latent, log_det, slope_old_layers[i][0], intercept_old_layers[i][0] = flow_layer.f_inverse(z_latent, 1-self.b_mask, h_encoding, 
                                                                                                        slope_old_layers[i][0], 
                                                                                                        intercept_old_layers[i][0],
                                                                                                        lambda_) 
            loglike_frame += log_det
            
            # the opposite mask b modifies the other features
            z_latent, log_det, slope_old_layers[i][1], intercept_old_layers[i][1] = flow_layer.f_inverse(z_latent, self.b_mask, h_encoding, 
                                                                                                        slope_old_layers[i][1], 
                                                                                                        intercept_old_layers[i][1],
                                                                                                        lambda_)
            loglike_frame += log_det
        ...
        return loglike_frame, slope_old_layers, intercept_old_layers
...
class FlowLayer(nn.Module):
    def __init__(self, frame_dim, esn_dim, hidden_layer_dim, num_hidden_layers, 
                 toeplitz, rescale_fn, device='cpu'):
    ...
    def f_inverse(self, x_frame, b_mask, h_encoding, slope=None, intercept=None, lambda_=None):
        ...
        if slope is None:
            ...
            # Clone the previous
            slope_old = slope.clone()
            intercept_old = intercept.clone()
            ...
            # Linear combination
            lambda_.data = torch.clamp_max(lambda_.data, 1.0)
            slope = lambda_ * slope_old + (1 - lambda_) * slope
            intercept = lambda_ * intercept_old + (1 - lambda_) * intercept
        
        z_latent = b_mask*x_frame \
                + (1-b_mask) * ((x_frame-intercept) * torch.exp(-slope))
        log_det = slope @ (b_mask.T - 1)  # final shape (batch_size, 1)

        return z_latent, log_det, slope, intercept
