...
from ..src import rnn_flows_modified
...
class DynRNN_flow(nn.Module):
    ...
    def __init__(self, num_categories=39, device='cpu', batch_size=64, frame_dim=40, model_type="rnn",
                encoding_dim=60, hidden_layer_dim=15, n_flow_layers=4, num_hidden_layers=1, 
        ...
        self.flow_model = rnn_flows_modified.NormalizingFlow(frame_dim=self.frame_dim,
    ...
    try:
        ...
        for epoch in range(nepochs):
            ...
            if tr_verbose == True and (((epoch + 1) % 5) == 0 or epoch == 0):
                ...
                print("Epoch: {}/{}, Training NLL:{:.6f}, Validation NLL:{:.6f},  Time_Elapsed:{:.4f} secs, Lambda:{:.4f}".format(epoch+1, 
                nepochs, tr_NLL_epoch, val_NLL_epoch, time_elapsed, dyn_rnn_flow_model.flow_model.lambda_.data.item()), file=orig_stdout)

                print("Epoch: {}/{}, Training NLL:{:.6f}, Validation NLL:{:.6f}, Time_Elapsed:{:.4f} secs, Lambda:{:.4f}".format(epoch+1, 
                nepochs, tr_NLL_epoch, val_NLL_epoch, time_elapsed, dyn_rnn_flow_model.flow_model.lambda_.data.item()))
